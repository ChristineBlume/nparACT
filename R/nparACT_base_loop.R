#' nparACT_base_loop
#'
#' Function returns IS, IV, RA, L5, M10 as well as L5 and M10 start times for multiple actigraphy
#' files. The function also returns a plot for hourly grand averages across 24 hours.
#'
...
#' format and a second column with actigraphy values. Input files can either be
...
#' @return Data frame containing IS, IV and RA values for each file. Besides this, L5 and M10 values are
#' given along with the respective start times
#' @examples
...
#' setwd(olddir)
#' @export
nparACT_base_loop <- function(path, SR, cutoff = 1, plot = FALSE, fulldays = TRUE, quiet = FALSE) {
  ...
  for (zz in 1:nofiles) {
    ...
    if (ncol(data) == 2) {
      # check if column is date, if not convert it. This should save some time
    ...
    nparACT_result[zz, 5] <- result_RA$L5_starttime
    nparACT_result[zz, 6] <- result_RA$M10
    nparACT_result[zz, 7] <- result_RA$M10_starttime
  }

...
nparACT_ISIV = function(data_hrs, bin_hr) {
  ...
  p <- 1440 / bin_hr
  l <- 60 / bin_hr
  mean_all <- mean.default(data_hrs[1:n, ])

  return_IS <- nparACT_IS(data_hrs, mean_all, bin_hr)

  IS <- return_IS[1]
  n <- return_IS[3]
  p <- return_IS[4]
  IV <- nparACT_IV(n, data_hrs, mean_all)
...
nparACT_IS = function(data_hrs, mean_all, bin_hr) {
  ...
  for (h in 1:24) {
    ...
    hrlymean <- mean.default(hrlydat, na.rm = TRUE)
...
nparACT_IV = function(n, data_hrs, mean_all) {
  ...
  IVnum <- sum(result_IVnum, na.rm = TRUE)
  IVnumerator <- n * IVnum
